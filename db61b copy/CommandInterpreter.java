...
class CommandInterpreter {
    ...
    Table selectClause(String tableName) {
        ...
        List<Condition> cond;
        List<TableIterator> iter = new ArrayList<>();
        List<String> coltitles = new ArrayList<>();

        _input.next("select");

        values.add(columnSelector());
        ...
        Table t1 = tableName();
        iter.add(t1.tableIterator());
        if (_input.nextIs(",")) {
            _input.next(",");
            Table t2 = tableName();
            iter.add(t2.tableIterator());
        }
        ...
        for (Column c : values) {
            if (c.getAlias() != null) {
                coltitles.add(c.getAlias());
            } else {
                coltitles.add(c.name());
            }

        }

        Table table = new Table(null, coltitles);


        select(table, values, iter, cond);
    ...
    Condition condition(List<TableIterator> iterators) {
        ...
        String rel = _input.next(RELATION);
        if (_input.nextIs(LITERAL)) {
            String l1 = literal();
            return new Condition(c1, rel, l1);
        } else  {
            Column c2 = columnSelector(iterators);
    ...
    private void select(Table table, ArrayList<Column> columns,
        ...
        if (iterators.size() > 2) {
            throw error("Should have 1 or 2 iterators");
        }

        if (iterators.size() == 1) {
            for (TableIterator r: iterators) {
                while (r.hasRow()) {
                    if (Condition.test(conditions)) {
                        Row temp = Row.make(columns);
                        table.add(temp);
                    }
                    r.next();
                }
